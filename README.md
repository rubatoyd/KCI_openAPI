...
<!-- usage:start -->
> 📈 **사용량** — 최근 14일 조회 **48**회(고유 10) · 클론 **410**회(고유 179) · 릴리스 자산 누적 다운로드 **206**
>
...
| 상황 | 처방 |
|---|---|
| `total > 100` — API 가 더 줄 수 없다 | `sort_dir` 를 뒤집어(asc↔desc) 반대쪽을 받아 합집합 |
| `total ≤ 100` 인데 `rows` 로 잘렸다 | `rows` 를 `total` 이상으로 올린다 (정렬 반전은 같은 레코드만 다시 온다) |

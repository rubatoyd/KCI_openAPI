...
### ✅ 출력 파일 경로 이탈 — **수정 완료**(2026-08-12, 커밋 `4dcd02e`)
...
경로에 직접 닿았다. 처방은 nl 과 동일하다: `exporters.safe_name()` 으로 호출부마다가 아니라
**`export()` 한 곳에서** 정규화하고(경로 구분자·`..`·제어문자·Windows 예약명·후행 점),
최종 경로가 `out_dir` 안인지 `resolve()` 로 **이중 확인**한다.
✅ 적용·검증 완료 — 재현 케이스 6종 전부 봉쇄(한글 파일명은 보존). 회귀는 `tests/test_output_path.py`.

### ✅ 적대적 검증은 **실제 HTTP 왕복**으로 — 이식 완료, 결함 3종 검출(2026-08-12)
`_call` 을 monkeypatch 하는 테스트는 **전송·인코딩 감지·재시도/백오프·URL 파라미터 인코딩·
resumptionToken 루프를 통째로 건너뛴다.** 로컬 `ThreadingHTTPServer` 로 실제 소켓을 태우자
이 저장소에서만 3종이 나왔다(`tests/test_adversarial_http.py`, 87 → **117**건).

- 🔴 **인증키가 MCP 응답까지 샜다** — `{"error": "<키> articleSearch 등록되지 않은 key 입니다."}`.
  원인은 전송 계층이 아니었다. **KCI 오류 봉투는 요청을 그대로 에코하고**(`<inputData><key>`),
  `check_rest_error` 가 **전 요소 텍스트를 이어붙여** 예외에 담고 있었다.
  ⚠️ 이 저장소는 `raise_for_status()` 가 URL 을 노출한다는 걸 알고 처음부터 피했고 주석까지
  남겼다 — **누출은 그 문이 아니라 옆문에서 났다.** `_safe` 의 독스트링은 "`_call` 단계에서
  인증키 포함 URL 은 이미 제거되므로 메시지에 키가 노출되지 않는다"고 **틀린 보증**을 하고 있었다.
  → ① `check_rest_error` 가 `inputData` 를 제외(요청 에코는 진단 가치도 없다)
    ② `KciClient._scrub` 2차 방어(오류 문구 자체에 키가 박혀 오는 경우)
    ③ ParseError→KciError 변환을 `_parse` 한 곳으로 모음(호출부 5곳 → 새 호출부에서 또 새는 것 방지).
- 🔴 **호출 폭주** — `display=1`·`max_records=1000` 이 **1,000회 요청**(display=5 는 200회).
  여러 페이지가 필요하면 전송 단위를 100 으로 올린다(결과 집합은 바뀌지 않는다).
  nl(499회)·scienceon(300회)과 **같은 결함의 세 번째 반복**. `citation` 에도 같이 적용했다.
- 🔴 **OAI 무한루프** — `list_identifiers` 의 종료 조건이 `len(out) < max_records` 뿐이라,
  서버가 토큰은 주면서 header 를 0건 주면 **영영 끝나지 않는다**. `list_records` 는 `max_pages`
  기본이 100,000 이라 throttle 0.5s 로 **13시간** — 사실상 같다.
  → **새 레코드가 없으면 중단**(+ 페이지 상한).
  ⚠️ 첫 처방은 **토큰 문자열 반복으로 순환을 판정**하는 것이었는데, 커서를 서버가 들고 있어
  매 페이지 같은 토큰을 주는 구현에서 **정상 수확 300건이 200건으로 잘렸다**(하네스가 잡았다).
  조용한 절단을 막겠다는 저장소에서 정반대 결과다 — 판정은 토큰이 아니라 **레코드**로 한다.
  중복 판정은 `contains` 필터 **앞에** 둔다(뒤에 두면 필터가 빡빡할 때 진전 없음으로 오판).

⚠️ **하네스 자신이 먼저 틀렸다(3건)** — 합성 OAI 레코드를 실제 `oai_kci` 구조와 다르게 만들어
`arti_id` 가 비고 **300건이 1건으로 접혔다**. 또 `kci_search(..., max_records=5)` 로 불러
**TypeError 가 `_safe` 에 먹히는 바람에 누출 테스트가 통과**했다(도구 인자는 `rows` 다).
→ 오류 경로 테스트에는 **"도구에 실제로 닿았는가"** 검사를 같이 넣는다.
※ `pytest-timeout` 을 들이는 대신 **가짜 서버가 상한에서 끊는다** — 무한루프 시 매달리지 않고
   호출수와 함께 실패한다(결정적).
